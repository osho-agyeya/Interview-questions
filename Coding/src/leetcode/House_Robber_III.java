...
Time complexity: \mathcal{O}(N)O(N) since we visit all nodes once.

Space complexity: \mathcal{O}(N)O(N) since we need stacks to do recursion, and the maximum depth of the recursion is the height of the tree, which is \mathcal{O}(N)O(N) in the worst case and \mathcal{O}(\log(N))O(log(N)) in the best case.
...
Time complexity: \mathcal{O}(N)O(N) since we run the helper function for all nodes once, and saved the results to prevent the second calculation.

Space complexity: \mathcal{O}(N)O(N) since we need two maps with the size of \mathcal{O}(N)O(N) to store the results, and \mathcal{O}(N)O(N) space for stacks to start recursion.
...
the example mapping with graph
...
\text{dp\_rob}_i = \text{tree}_i + \sum_{\text{child} \in \text{graph}_i}\text{dp\_not\_rob}_{\text{child}}dp_rob 
i
​
 =tree 
i
...
\text{dp\_not\_rob}_i = \sum_{\text{child} \in \text{graph}_i}\max(\text{dp\_rob}_{\text{child}}, \text{dp\_not\_rob}_{\text{child}})dp_not_rob 
i
​
 =∑ 
child∈graph 
...
Time complexity: \mathcal{O}(N)O(N) since we visit all nodes once to form the tree-array, and then iterate two DP array, which both have length \mathcal{O}(N)O(N).

Space complexity: \mathcal{O}(N)O(N) since we need an array of length \mathcal{O}(N)O(N) to store the tree, and two DP arrays of length \mathcal{O}(N)O(N). Also, the sizes of other data structures in code do not exceed \mathcal{O}(N)O(N).
	 * 
	 */
